...
// 3. Helper Functions
async function getBrowserUrl(sessionId, apiKey) {
    try {
        // 发起请求
        const response = await fetch(`https://cloud.yunlogin.com/v2/cloudbrowser/api/session/start?apiKey=${apiKey}&sessionId=${sessionId || ''}`, {
...
async function createNewBrowserSession(sessionId, apiKey) {
                ...
                try {
                    ...
                    return {
                        content: [
                            {
                                type: "text",
                                text: `Failed to Evaluated script: ${args.script}: ${error.message}`,
                            },
                        ],
                        isError: true,
                    };
